#ifndef IMGUI_MARKUP_SRC_IMGUI_MARKUP_PARSER_LEXER_H_
#define IMGUI_MARKUP_SRC_IMGUI_MARKUP_PARSER_LEXER_H_
...
struct ExpectedAttributeID : public LexerException
{
    ExpectedAttributeID(LexerToken token)
        : LexerException("Expected attribute ID after '@'", token,
                         ParserResultType::kExpectedAttributeID)
    { }
};
...
private:
    ...
    */
    ...
    /**
     * Assumes that the current char is an '@' symbol, followed
     * by the ID of the attribute that is being referenced.
     *
     * @return LexerToken token with the ID of the attribute beeing referenced
     *         as its data.
     */
    LexerToken CreateReference();
    ...
    /**
     * Checks if the current char is the start of a bool.
     * true and false are the only valid bool values and have to be written
     * in lowercase.
     *
     * @param temnp_buffer - Buffer that will recive the bool value.
     *                       This is used so that the bool value
     *                       does not have to get generated multiple times
     *                       between the check and the creation function..
     */
    bool IsBool(std::string& temp_buffer) const;

    /**
     * Assumes that the current char is the start of a bool value and creates
     * a bool token.
     * See IsBool() for more information.
     *
     * @param value - true/false value generated by the IsBool function.
     */
    LexerToken CreateBool(std::string value);
